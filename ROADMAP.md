...
## Недавно сделано (v0.47.0–v0.52.0)
...
- **At-rest шифрование секретов клиента — сделано (v0.42.0):** AES-256-GCM, KEK из env, миграция 037 (enc/nonce-колонки) + идемпотентный бэкфилл. **Drop plaintext-колонок — сделано (#175, миграция 047):** чтение/запись только через ciphertext; guard в 047 (RAISE если есть un-backfilled секрет) + авто-backfill при старте ДО миграций (guard не срабатывает на нормальном старте). Остаётся: **ротация KEK (#178)**; `webhook_secret` хеширование
