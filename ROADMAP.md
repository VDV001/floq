...
### Webhook integrations — сделано (#181 + #206)
Outgoing webhooks на ключевые события для интеграции с CRM/Zapier/etc. без копирования данных. **#181 готов (v0.63.0–v0.69.0):** delivery-инфра `internal/webhooks` (мульти-endpoint + outbox-таблица + cron-воркер + 2-слойная SSRF-защита + HMAC-подпись + секрет encrypted-at-rest), Settings UI, эмиссия 6 событий (`lead.created`/`lead.qualified`/`lead.archived`/`pending_reply.approved`/`sequence.completed`), + техдолг (due-индекс #198, active-toggle #201, sequence.completed #197, transactional outbox #199). **#206 idempotent intake готов (v0.70.0–v0.72.0):** все 6 poller-originated событий стали транзакционными (fail-closed) — email lead.created (A), telegram lead.created через ручной offset-gating (B), auto-qualification через durable retryable воркер + миграция 053 `lead_qualification_jobs` (C). **#208 intake retry-cap/quarantine готов (v0.73.0):** in-memory cap фейлов per source key (email UID / telegram update_id), на cap — quarantine (источник потребляется + метрика `inbox_intake_quarantined_total`), без миграции. **#212 готов (v0.74.0–v0.75.0):** retention-GC терминальных строк обеих очередей через общий `internal/retention.Cron` (part 1, v0.74.0, без миграции) + multi-worker leased claim — колонка `locked_until` (миграция 054), claim-one-at-a-time под `FOR UPDATE SKIP LOCKED`, оба воркера безопасны на нескольких инстансах (part 2, v0.75.0). Весь кластер #181/#206/#208/#212 закрыт.

### Auto-enrichment — сделано (#182, v0.55.0–v0.58.0)
Обогащение карточки компании по email-domain'у. P1: website-scraping (table-as-queue + cron, 2-слойная SSRF-защита). P2: LLM-overlay industry/company_size (за флагом `ENRICHMENT_LLM_ENABLED`). P3: официальный DaData.ru API (вместо скрейпинга реестров — решает match-without-INN без SSRF-поверхности), домен VO `INN`/`OGRN` с checksum. Observability: Prometheus `enrichment_registry_attempts_total`. Платные интеграции (Clearbit, Apollo) — отдельный gate, не начато.
