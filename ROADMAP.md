...
Outgoing webhooks на ключевые события для интеграции с CRM/Zapier/etc. без копирования данных. **#181 готов (v0.63.0–v0.69.0):** delivery-инфра `internal/webhooks` (мульти-endpoint + outbox-таблица + cron-воркер + 2-слойная SSRF-защита + HMAC-подпись + секрет encrypted-at-rest), Settings UI, эмиссия 6 событий (`lead.created`/`lead.qualified`/`lead.archived`/`pending_reply.approved`/`sequence.completed`), + техдолг (due-индекс #198, active-toggle #201, sequence.completed #197, transactional outbox #199). **#206 idempotent intake готов (v0.70.0–v0.72.0):** все 6 poller-originated событий стали транзакционными (fail-closed) — email lead.created (A), telegram lead.created через ручной offset-gating (B), auto-qualification через durable retryable воркер + миграция 053 `lead_qualification_jobs` (C). Остаётся (не блокеры): retry-cap/dead-letter (#208), retention-GC + multi-worker claim (#212).
