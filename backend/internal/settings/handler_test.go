...
func TestHandler_TestAI_Auth_FriendlyMessage(t *testing.T) {
	...
	assert.False(t, resp["success"].(bool))
	msg, _ := resp["error"].(string)
	assert.Contains(t, msg, "ключ",
		"a rejected key must yield an actionable RU hint about the API key; got: %q", msg)
	assert.NotContains(t, msg, "ai auth", "raw sentinel text must not leak to the user")
}

func TestHandler_TestAI_RateLimit_FriendlyMessage(t *testing.T) {
	repo := newMockSettingsRepo()
	uc := NewUseCase(repo, &mockTelegramValidator{})
	r := chi.NewRouter()
	tester := func(_ context.Context, _, _, _ string) (string, error) {
