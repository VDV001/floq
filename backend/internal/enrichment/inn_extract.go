...
// (an order id, a phone) must NOT be treated as an INN. A leading boundary
// (start-of-string or a non-letter) keeps "инн" embedded in a word (e.g.
// «магазинн») from triggering. The checksum in domain.NewINN is the second guard.
var innLabelRe = regexp.MustCompile(`(?i)(?:^|[^\p{L}])инн\D{0,5}(\d{10,12})`)
