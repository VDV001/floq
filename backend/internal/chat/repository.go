...
func (r *Repository) FetchStats(ctx context.Context, userID uuid.UUID) (*userStats, error) {
	...
	err := r.q.QueryRow(ctx,
		`SELECT COUNT(*) FROM leads WHERE user_id = $1 AND archived_at IS NULL`, userID).Scan(&s.TotalLeads)
	if err != nil {
	...
	rows, err := r.q.Query(ctx,
		`SELECT status::text, COUNT(*) FROM leads WHERE user_id = $1 AND archived_at IS NULL GROUP BY status`, userID)
	if err != nil {
	...
	recentRows, err := r.q.Query(ctx,
		`SELECT contact_name, COALESCE(company, ''), status::text, created_at FROM leads WHERE user_id = $1 AND archived_at IS NULL ORDER BY created_at DESC LIMIT 10`,
		userID)
	if err != nil {
