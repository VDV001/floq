...
**Ops-caveat:** клиент enrichment **всегда** дилит напрямую и игнорирует `PROXY_URL` (guard всегда активен — безопаснее). Следствие: деплой, у которого единственный исходящий маршрут — прокси (нет direct egress), не достучится до внешних сайтов и enrichment будет тихо фейлиться. CGNAT `100.64/10` и спец-диапазоны пока не блокируются (`IsPrivate` = RFC1918+ULA) — Phase-2.
